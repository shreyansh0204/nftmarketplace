  ...
  <div className='flex justify-center'>
    <div className='px-4'style={{maxWidth:'1600px'}}>
      <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 pt-4'>
        {
          nfts.map((nft,i)=>(
            <div key={i} className='border shadow rounded-xl overflow-hidden mx-5 ny-5'>
               <Image src ={nft.image} alt={nft.name} width={300} height={200} placeholder="blur" blurDataURL='/placeholder.png' layout='responsive'/> 
                ...
                <button className='w-full bg-pink-500 tect-white font-bold py-2 px-12 rounded' onClick={()=>buyNFT(nft)}>Buy Now </button>
              </div>    
              </div>
          ))
        }
